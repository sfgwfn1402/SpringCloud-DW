...
import com.dwcloud.core.config.HttpConsumerProperties;
import com.dwcloud.core.model.HttpDomain;
import com.dwcloud.core.util.BinderUtils;
...
public class HttpConsumerPostProcessor implements BeanClassLoaderAware, EnvironmentAware, BeanFactoryPostProcessor, ApplicationContextAware {
    ...
    private OkHttpClient buildOkHttpClient(ConfigurableEnvironment environment) {
        ...
        ThreadPoolExecutor executor = new ThreadPoolExecutor(properties.getCoreThreads(), properties.getMaxThreads(), properties.getKeepAliveTime(), TimeUnit.SECONDS, new SynchronousQueue<>(), Util.threadFactory("OkHttp Custom Dispatcher", false));
    ...
    private void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory, BeanDefinitionRegistry registry) {
        ...
        for (String beanName : httpClientBeanDefinitions.keySet()) {
            if (context.containsBean(beanName)) {
                throw new IllegalArgumentException("[HttpConsumer Starter] Spring context already has a bean named " + beanName + ", please change @HttpConsumer field name.");
            }
    ...
    private void parseElement(Field field) {
        ...
        BeanDefinitionBuilder beanDefinitionBuilder = BeanDefinitionBuilder.rootBeanDefinition(HttpConsumerProxyFactoryBean.class).addConstructorArgValue(interfaceClass).addConstructorArgValue(httpDomain).addConstructorArgValue(okHttpClient);
