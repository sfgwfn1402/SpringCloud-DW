...
import com.dwcloud.auth.cloud.gateway.exception.RequestAccessDeniedHandler;
import com.dwcloud.auth.cloud.gateway.exception.RequestAuthenticationEntryPoint;
import com.dwcloud.auth.cloud.gateway.filter.CorsFilter;
...
public class SecurityConfig {
    ...
    SecurityWebFilterChain webFluxSecurityFilterChain(ServerHttpSecurity http) throws Exception {
        //1. 认证过滤器，放入认证管理器tokenAuthenticationManager。（认证过滤器，利用认证管理器进行令牌校验）
        ...
        http
                ...
                //3. 鉴权管理器配置和异常提示信息配置，其他的请求必须鉴权，使用鉴权管理器
                ...
                //5. 认证过滤器配置，token的认证过滤器，用于校验token和认证
